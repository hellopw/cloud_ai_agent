...
- 目录结构：`backend/cmd/`, `backend/internal/`, `backend/migrations/`
- 依赖：gin 或 chi（路由）、mattn/go-sqlite3（数据库）、golang-migrate（migration）
- Git 仓库：`https://github.com/hellopw/cloud_ai_agent.git`，分支 `main`
- 健康检查端点 `GET /api/health`
...
### 1.7 Phase 1 完成提交
- `git push origin main`，提交信息 `Phase 1: 基础骨架完成`
...
- 生成的 Dockerfile 内容：
  ...
  - `COPY pi-skills/ pi-prompts/ extensions/`
  - `COPY server.js`
  - `ENV` 占位（LLM provider keys 在运行时注入）
  - `EXPOSE 3000`
  - `CMD ["node", "server.js"]`
...
### 2.6 Phase 2 完成提交
- `git push origin main`，提交信息 `Phase 2: Agent 生命周期完成`
...
### 3.5 Phase 3 完成提交
- `git push origin main`，提交信息 `Phase 3: 实例与对话完成`
...
### 4.4 Phase 4 完成提交
- `git push origin main`，提交信息 `Phase 4: 完善与测试完成`
...
每个 Phase 完成时 push 到 `https://github.com/hellopw/cloud_ai_agent.git`，中间子任务可独立提交。

| 阶段 | 提交信息 |
|---|---|
| Phase 1 | `Phase 1: 基础骨架完成` |
| Phase 2 | `Phase 2: Agent 生命周期完成` |
| Phase 3 | `Phase 3: 实例与对话完成` |
| Phase 4 | `Phase 4: 完善与测试完成` |
