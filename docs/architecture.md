...
│  Agents | Agent Teams | Instances               │
│  Models | Resources | Chat UI                   │
└──────────────────┬──────────────────────────────┘
                   │ REST + WebSocket
┌──────────────────┴──────────────────────────────┐
│           Go Backend (:8080, 单体)               │
...
│  │  server.js: Express + pi SDK (:3000)       │ │
│  │  bind mount: /workspace                    │ │
│  └────────────────────────────────────────────┘ │
│  ┌────────────────────────────────────────────┐ │
│  │ Team Agent Container                       │ │
│  │  team-server.js: Multi-Agent (:3000)        │ │
│  │  Leader + N Workers, delegate_task tool    │ │
│  │  bind mount: /workspace                    │ │
│  └────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────┘
```
...
instances        (id, agent_id, team_id, container_id, host_port, status, error_msg,
                  created_at, updated_at)
...
-- Team tables
agent_teams      (id, name, template_id, repo_url, git_username, git_password,
                  branch, image_tag, status, error_msg, created_at, updated_at)
team_members     (id, team_id -> agent_teams, name, role, agent_template_id,
                  provider_config_id, system_prompt_override, sequence,
                  created_at, updated_at)
team_member_prompts (team_member_id -> team_members, prompt_id, PK)
team_member_skills  (team_member_id -> team_members, skill_id, PK)
team_member_tools   (team_member_id -> team_members, tool_id, PK)
agent_team_prompts  (team_id -> agent_teams, prompt_id, PK)
agent_team_skills   (team_id -> agent_teams, skill_id, PK)
```
...
| `/api/agent-teams` | GET/POST | 团队列表/创建 |
| `/api/agent-teams/{id}` | GET/PUT/DELETE | 团队 CRUD（仅 draft/failed 可编辑） |
| `/api/agent-teams/{id}/build` | POST | 触发团队构建（异步） |
...
container-wrapper/src/team-server.js — Express 服务，3000 端口，多 Agent 协作：

| 端点 | 说明 |
|---|---|
| `GET /status` | 返回所有成员名称、角色、就绪状态 |
| `POST /chat` | SSE 流，始终路由到 Leader |
| `POST /abort` | 中断 Leader 当前操作 |
| `POST /internal/agents/:name/chat` | 内部：Worker 执行（delegate_task 调用） |

Leader 内置 `delegate_task` 工具，参数 `worker` + `task`，同步调用 Worker 并返回结果。

所有成员共享 7 个内置 tools（bash, read_file, write_file, edit_file, search_content, list_files, git）。每个成员独立 LLM 配置和上下文。

### Team 构建流程

POST /api/agent-teams/{id}/build：

1. Git clone 仓库
2. 生成 Team Dockerfile（team_dockerfile.go）
3. 复制 team-server.js / mcp-client.js
4. 为每个 member 创建子目录，写入 tools/prompts/skills（Team 级 + 成员级合并）
5. 从 ProviderConfig 读取各成员 LLM 配置
6. 写入 team-manifest.json（含所有成员 api_key 等）
7. docker build + 状态更新
...
| `/agents` | AgentsPage | 列表、创建、编辑、构建、启动、查看日志/Dockerfile、删除 |
| `/agent-teams` | AgentTeamsPage | 团队列表、构建、启动、查看日志、删除 |
| `/agent-teams/new` | AgentTeamEditPage | 创建团队 |
| `/agent-teams/:id` | AgentTeamEditPage | 编辑团队（成员、绑定配置） |
| `/instances` | InstancesPage | 列表、启动（选 Model）、Chat 入口、Stop、团队标识 |
...
| `/resources` | ResourcesPage | 资源管理（Git/Database/Knowledge Base） |
