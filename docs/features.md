...
### 9. Chat 对话
- WebSocket 连接，实时流式输出；断开自动重连（3秒间隔）
- WebSocket 被公司代理阻止时自动降级为 HTTP SSE 回退
- 支持消息类型：user, assistant, tool, system, thinking
- Anthropic 协议 thinking 内容以 `[Thinking]` 前缀展示
- Tool call 卡片展示（tool 名称、参数）
- 流式内容逐字渲染，光标闪烁动画
- 显示当前 provider 和 model 信息
- **Chat 页面顶部展示实例关联的 Tools/Skills/Prompts 列表**，点击可展开查看详情
- 连接状态指示及 WebSocket readyState 详情
- 未识别的事件类型通过 default 分支兜底展示，避免静默丢弃
